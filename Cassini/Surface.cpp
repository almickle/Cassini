...
void
Surface::Clear(Color fillValue)
{
  memset(pBuffer.get(), fillValue.dword, width * height * sizeof(Color));
}

void
Surface::PutPixel(unsigned int x, unsigned int y, Color c)
{
  assert(x >= 0);
  assert(y >= 0);
  assert(x < width);
...
Surface::Color
Surface::GetPixel(unsigned int x, unsigned int y) const
{
  assert(x >= 0);
  assert(y >= 0);
  assert(x < width);
...
unsigned int
Surface::GetWidth() const
{
  return width;
}

unsigned int
Surface::GetHeight() const
{
  return height;
}

Surface::Color*
Surface::GetBufferPtr()
{
  return pBuffer.get();
}

const Surface::Color*
Surface::GetBufferPtr() const
{
  return pBuffer.get();
}

const Surface::Color*
Surface::GetBufferPtrConst() const
{
  return pBuffer.get();
}

Surface
Surface::FromFile(const std::string& name)
{
...
{
  auto GetEncoderClsid = [&filename](const WCHAR* format,
                                     CLSID* pClsid) -> void {
...
void
Surface::Copy(const Surface& src)
{
...
const char*
Surface::Exception::GetType() const
{
  return "Chili Graphics Exception";
}

const std::string&
Surface::Exception::GetNote() const
{
  return note;
}